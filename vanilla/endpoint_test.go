...
import "github.com/milochristiansen/autorest/vanilla"
...
func TestBasicFunction(t *testing.T) {
	...
	// Read
	r, err = http.NewRequest("GET", "/test?id=1", nil)
	if err != nil {
		t.Fatal(err)
	}
	w = httptest.NewRecorder()
	...
	// Update
	r, err = http.NewRequest("PUT", "/test?id=1", strings.NewReader(`{"Int": 10}`))
	if err != nil {
		t.Fatal(err)
	}
	w = httptest.NewRecorder()
	...
	// Delete
	r, err = http.NewRequest("DELETE", "/test?id=1", nil)
	if err != nil {
		t.Fatal(err)
	}
	w = httptest.NewRecorder()
